...
export class LikesService {
  ...
  async likeItem(likeableId: string, likeableType: 'Post' | 'Comment', userId: string): Promise<boolean> {
    const { model, item } = await this.getLikeableItem(likeableId, likeableType);
    ...
    try {
      ...
      if (like === null) { // A new like was created
        newLikesCount++;
        await model.updateOne({ _id: likeableId }, { $inc: { likesCount: 1 } }, { session });
      }
      ...
      if (like === null) {
        ...
        if (item.author.toString() !== userId) {
          this.notificationsService.create({
            ...
            sender: userId,
            type: likeableType === 'Post' ? NotificationType.POST_LIKE : NotificationType.COMMENT_LIKE, // Assuming COMMENT_LIKE exists
  ...
  async unlikeItem(likeableId: string, likeableType: 'Post' | 'Comment', userId: string): Promise<boolean> {
    const { model } = await this.getLikeableItem(likeableId, likeableType);

    const session = await this.likeModel.db.startSession();
    session.startTransaction();
    try {
      const result = await this.likeModel.deleteOne({ likeableId, likeableType, user: userId }).session(session);

      if (result.deletedCount > 0) {
        const updatedItem = await model.findOneAndUpdate({ _id: likeableId }, { $inc: { likesCount: -1 } }, { session, new: true });
        // Publish the update
        this.pubSub.publish('LIKES_UPDATED', {
          likesUpdated: { likeableId, likeableType, likesCount: updatedItem.likesCount }
        });
      }
   ...
   */
  async findUserLikesForItems(keys: readonly { likeableId: string, likeableType: string }[], userId: string): Promise<Set<string>> {
  ...
  private async getLikeableItem(id: string, type: 'Post' | 'Comment'): Promise<{ model: Model<PostDocument | CommentDocument>, item: PostDocument | CommentDocument }> {
    ...
    const item = await model.findById(id).select('_id author likesCount').lean();
    if (!item) {

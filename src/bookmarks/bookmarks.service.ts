...
import { Bookmark, BookmarkableType, BookmarkDocument } from './schemas/bookmark.schema';
...
export class BookmarksService {
    ...
    async addBookmark(
        ...
        try {
            await this.bookmarkModel.updateOne(query, { $setOnInsert: query }, { upsert: true });
            return true;
        } catch (error) {
     ...
     */
    async findUserBookmarkedItems(keys: readonly BookmarkLoaderKey[]): Promise<Set<string>> {
